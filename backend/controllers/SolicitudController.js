...
export class SolicitudController {
    static async createSolicitud(req, res) {
        ...
        try {
            ...
            if (donanteSocketId) {
                ...
                console.log(`Notificación de solicitud enviada a ${donante.nombre} en socket ${donanteSocketId}`);
            }
    ...
    static async aceptarSolicitudYProponerHorario(req, res) {
        ...
        if (!horarioEntregaPropuestaPorDonante || !fechaPropuesto) {
            return res.status(400).json({ message: "Debe proporcionar una propuesta de fecha y hora." });
        }
        ...
        try {
            ...
            await Donacion.findByIdAndUpdate(solicitudAceptada.donacionId, { estadoPublicacion: 'PENDIENTE-ENTREGA' }).session(session);
            ...
            // --- LÓGICA ADICIONAL: Rechazar automáticamente otras solicitudes para la misma donación ---
    ...
    static async rechazarSolicitud(rea, res){
        const {solicitudId} = req.params;
    ...
    static async cancelarSolicitud(req, res) {
        try {
            ...
            if (solicitante.clerkUserId !== solicitanteClerkId) {
                return res.status(403).json({ message: 'No tienes permiso para cancelar esta solicitud.' });
            }

            // Un receptor solo puede cancelar si aún está pendiente de aprobación
            if (solicitud.estadoSolicitud !== 'PENDIENTE_APROBACION') {
                return res.status(400).json({ message: 'No puedes cancelar una solicitud que ya ha sido gestionada.' });
            }
