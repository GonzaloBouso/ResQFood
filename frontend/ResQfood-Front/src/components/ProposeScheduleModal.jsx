...
const ProposeScheduleModal = ({ solicitud, onClose, onSubmit, isSubmitting }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Pasa el ID de la solicitud y la propuesta al componente padre
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="date" id="fecha" value={fecha} onChange={e => setFecha(e.target.value)} required className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary" min={new Date().toISOString().split('T')[0]}/>
          </div>
          <div className="flex gap-4">
            <div className="w-1/2">
              <label htmlFor="horaInicio" className="block text-sm font-medium text-gray-700">Desde las</label>
              <input type="time" id="horaInicio" value={horaInicio} onChange={e => setHoraInicio(e.target.value)} required className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary"/>
            </div>
            <div className="w-1/2">
              <label htmlFor="horaFin" className="block text-sm font-medium text-gray-700">Hasta las</label>
              <input type="time" id="horaFin" value={horaFin} onChange={e => setHoraFin(e.target.value)} required className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary"/>
            </div>
          </div>
