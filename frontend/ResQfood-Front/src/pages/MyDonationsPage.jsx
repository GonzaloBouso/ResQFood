// frontend/src/pages/MyDonationsPage.jsx (CÓDIGO COMPLETO Y CORREGIDO)
...
const SolicitudesList = ({ solicitudes, onAcceptClick, onReject, isSubmitting }) => {
    ...
    if (pendientes.length === 0) {
        // Mejora: Si no hay pendientes, revisamos si hubo un rechazo de horario reciente
        ...
        if (rechazoReciente) {
            return (
                <div className="p-3 bg-red-50 border-t text-red-700 text-xs flex items-center gap-2">
                    <XCircle size={16}/>
                    <span>El horario propuesto a <strong>{rechazoReciente.solicitanteId?.nombre}</strong> fue rechazado. La donación está disponible nuevamente.</span>
                </div>
            );
        }
